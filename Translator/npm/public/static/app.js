...
async function translate() {
    ...
    try {
        ...
        // Automatically speak the translated text after successful translation (if autoSpeak is enabled)
        const autoSpeakToggle = document.getElementById('autoSpeakToggle');
        const autoSpeakEnabled = autoSpeakToggle ? autoSpeakToggle.checked : true;
        
        if (autoSpeakEnabled && selectedVoice && lastTranslatedText.length > 0) {
            ...
            // Save without audio if no voice selected or autoSpeak is disabled
            if (selectedModel) {
                const langCode = selectedModel.lang_code || selectedModel.name.split('-')[1] || 'unknown';
                await saveTranslationToHistory(langCode, inputText, translatedContent, selectedModel);
            }
...
async function loadConfig() {
    try {
        ...
        if (sourcesInfo && config.sources) {
            let html = '';
            if (config.sources.translationModels) {
                const tm = config.sources.translationModels;
                html += `<p><strong>Translation Models:</strong> ${tm.name} by ${tm.organization}</p>`;
                html += `<p>License: ${tm.license}</p>`;
                if (tm.url) {
                    html += `<p><a href="${escapeHtml(tm.url)}" target="_blank">${escapeHtml(tm.url)}</a></p>`;
                }
            }
            if (config.sources.ttsVoices) {
                const tv = config.sources.ttsVoices;
                html += `<p style="margin-top: 15px;"><strong>TTS Voices:</strong> ${tv.name} by ${tv.organization}</p>`;
                html += `<p>License: ${tv.license}</p>`;
                if (tv.url) {
                    html += `<p><a href="${escapeHtml(tv.url)}" target="_blank">${escapeHtml(tv.url)}</a></p>`;
                }
                if (tv.voicesUrl) {
                    html += `<p><a href="${escapeHtml(tv.voicesUrl)}" target="_blank">Voices on HuggingFace</a></p>`;
                }
            }
        ...
        if (contactInfo && config.contact) {
            ...
            if (config.contact.email) {
                html += `<p><a href="mailto:${escapeHtml(config.contact.email)}">${escapeHtml(config.contact.email)}</a></p>`;
            }
            if (config.contact.website) {
                html += `<p><a href="${escapeHtml(config.contact.website)}" target="_blank">${escapeHtml(config.contact.website)}</a></p>`;
            }
            if (config.contact.github) {
                html += `<p><a href="${escapeHtml(config.contact.github)}" target="_blank">GitHub</a></p>`;
            }
            if (!html) {
        ...
        if (knownBugs && config.knownBugs && Array.isArray(config.knownBugs)) {
            if (config.knownBugs.length > 0) {
                let html = '<ul>';
                config.knownBugs.forEach(bug => {
                    html += `<li>${escapeHtml(bug)}</li>`;
                });
                html += '</ul>';
                knownBugs.innerHTML = html;
            } else {
                knownBugs.innerHTML = '<p>No known issues at this time</p>';
            }
        }
    } catch (error) {
        console.error('Failed to load config:', error);
        // Hide footer sections if config fails to load
